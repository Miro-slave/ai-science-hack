...
<style>
    :root {
        ...
        --color-accent: #a11d24;
        --color-accent-dark: #7d151b;
        --color-accent-soft: #f6e9ea;
    }
    html, body, [class*="css"] {
    ...
    .app-hero {
        background: linear-gradient(135deg, #2b2b2b 0%, #3c4046 100%);
        border-radius: 6px;
        padding: 28px 34px;
        margin-bottom: 8px;
...
def create_pdf_report(
    ...
    heading_style = ParagraphStyle(
        ...
        fontSize=14,
        leading=18,
        textColor=colors.HexColor("#a11d24"),
        spaceBefore=12,
        spaceAfter=8,
    )
...
def section_title(text: str) -> None:
    ...
    st.markdown(
        f'<div class="section-title"><span class="accent"></span>{text}</div>',
        unsafe_allow_html=True,
    )
# -----------------------------------------------------------------------------
# Inference
...
st.markdown(
    """
<div class="app-hero">
    <h1>Система автоматической классификации руд по OM-изображениям</h1>
    <p>Геологический дашборд анализа шлифов и определения доли талька</p>
</div>
""",
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    section_title("Настройки обработки")
    ...
    section_title("История анализов")
    if st.session_state.processing_history:
        for i, item in enumerate(st.session_state.processing_history[-5:]):
            st.markdown(
                f'<div class="history-item"><strong>{i + 1}.</strong> '
                f'{item["filename"]}<br>{item["classification"]}</div>',
                unsafe_allow_html=True,
            )
    else:
...
with col1:
    section_title("Загрузка изображения")
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.analysis_in_progress:
            st.warning(
                "Анализ уже выполняется. Пожалуйста, дождитесь завершения текущего анализа.",
            )
            st.button(
                "Провести анализ",
                type="primary",
    ...
    if (
        ...
        with st.spinner("Выполняется анализ..."):
            ...
            # Определяем метку класса и описание.
            if talc_content > 10:
                class_emoji = ""
                description = (
...
if st.session_state.current_result:
    ...
    st.markdown("---")
    section_title("Результаты анализа")
    ...
    section_title("Детальная таблица")
    ...
    # -------------------------------------------------------------------------
    st.markdown("---")
    section_title("Режим экспертной проверки")
    if CANVAS_AVAILABLE and st.session_state.original_image is not None:
        ...
        with col_btn1:
            if st.button("Очистить холст", use_container_width=True):
                st.session_state.canvas_key = str(uuid.uuid4())
                st.session_state.expert_markup_bytes = None
                st.rerun()
        with col_btn2:
